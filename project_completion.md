...
| **6. Customer Profiles** | **20% Done** | 10% | 2% | Basic profile mapping exists. *(Remaining: address book management).* |
...
To bring the project status to **87%**, the next focus is implementing **Customer Address Book & Booking Locations**:
1. **Customer Address Book (CRUD)**: `POST`, `GET`, `DELETE` address endpoints for customers.
2. **Booking Location Linking**: Pass `addressId` on booking creation and store the reference in the database.
3. **Provider Discovery Location**: Update the provider's booking discovery API to show the job address/location.
